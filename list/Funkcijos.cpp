...
void skaitymas (studentas &temp, list <studentas> &mas, double &skaitym, string failodydis) {
    ...
    stringstream ss(splited.front()); // pirmoji perskelta eilute yra pradine ivestis
    string word;
    ...
    int pazymys;
    std::list<string>::iterator it = splited.begin();
...
double mediana (studentas& temp) {
    ...
    int vid=size/2;
    temp.paz.sort();
    auto it = std::next(temp.paz.begin(), vid);
    if (size % 2 == 0) {
        mediana = double((*std::prev(it) + *it)) / 2;
    } else mediana = *it;
    return mediana;
}
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    galutinis = a == '1'? galutinis="(Vid.)" : galutinis="(Med.)";
       ...
       if(a==0){ // i terminal isvesti
        ...
        cout<<endl;
        for(auto& i : temp) {
            cout << setw(15) << i.vardas << setw(15) << i.pavarde << setw(19) << fixed << setprecision(2) << vidurkis(i, x) << endl;
        }
       }
       else{ // i txt isvesti
...
void rusiavimas (list <studentas> &temp, list <studentas> &vargsas, list <studentas> &kietas, string failodydis, double &rusiavim) {
    ...
    vargsas.sort(palyginimas2);
    kietas.sort(palyginimas2);
...
void isvedimas2 (list <studentas> &temp, string failodydis, string tipas, double &isved) {
    ...
    cout << endl;
    for(auto& i : temp) {
        out << setw(15) << i.vardas << setw(15) << i.pavarde << setw(19) << fixed << setprecision(2) << i.bendr << endl;
        }
    out.close();
